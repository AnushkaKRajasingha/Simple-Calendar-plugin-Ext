...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (seg.isStart) {
			...
			timeDuration = Math.ceil(Math.abs(event.start - event.end) / 36e5) + ' hours';
			timeRange = new Date(event.start).toLocaleString('en-US', {timeZone: 'Asia/Hong_Kong', hour: 'numeric' , minute:'numeric' , hour12: true }) +
				' - ' + new Date(event.end).toLocaleString('en-US', {timeZone: 'Asia/Hong_Kong', hour: 'numeric' , minute:'numeric' , hour12: true });
			if (timeText) {
		...
		titleHtml =
			'<div class="fc-title">' +
				(htmlEscape(timeDuration+event.title || '') || '&nbsp;') + // we always want one line of height
			'<span class="fc-time-range" style="display: block">' +
				timeRange +
			'</span></div>';
		//console.log(event);
		titleHtml = '<span class="fc-time-range" style="display: block">' +
			timeRange +
			'</span></div>'; // did on client request ;
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				'<div class="fc-content">' +
					(event.title ?
						'<div class="fc-title">' +
							htmlEscape(timeDuration+event.title) +
						'</div>' :
						''
						) +
					(timeText ?
...
Calendar.defaults = {
	...
	buttonText: {
		...
		today: simcal_fullcal.cal_nav_today,
		month: simcal_fullcal.cal_nav_months,
		week: simcal_fullcal.cal_nav_weeks,
		day: simcal_fullcal.cal_nav_days
	},
